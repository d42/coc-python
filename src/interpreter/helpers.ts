...
import { workspace, ConfigurationTarget, Uri } from 'coc.nvim'
...
export class InterpreterHelper implements IInterpreterHelper {
  ...
  public getActiveWorkspaceUri(resource: Resource): WorkspacePythonPath | undefined {
    ...
    if (Array.isArray(workspaceService.workspaceFolders) && workspaceService.workspaceFolders.length === 1) {
      return { folderUri: Uri.parse(workspaceService.workspaceFolders[0].uri), configTarget: ConfigurationTarget.Workspace }
    }

    if (resource) {
      const workspaceFolder = workspaceService.getWorkspaceFolder(resource)
      if (workspaceFolder) {
        return { configTarget: ConfigurationTarget.Workspace, folderUri: Uri.parse(workspaceFolder.uri) }
      }
    }
    ...
    if (doc != null) {
      const workspaceFolder = workspaceService.getWorkspaceFolder(Uri.parse(doc.uri))
      if (workspaceFolder) {
        return { configTarget: ConfigurationTarget.Workspace, folderUri: Uri.parse(workspaceFolder.uri) }
      }
    }
